...
% The title
\title[short title]{Non-smooth Control Barrier Functions for Safety Critical Systems}
\subtitle{A Gentle Introduction}

\author{Chirayu Salgarkar \and Zachary Medlin \and Dr. Makhin Thitsa}
...
\bigbreak
 ...
 \begin{frame}{Background}  
  ...
  \begin{itemize}
   ...
   \item In classical control, most problems and situations are treated as a single-input single-output (SISO) system. That does not allow us to to see anything with respect to the interior structure of the system. 
...
\begin{frame}{Background}
    ...
    \begin{block}{State-determined Systems}
        A \textit{state-determined system} has the characteristic that given the mathematical description of the system set of variables, as well as the initial time conditions of such variables, and system inputs for time $t > t_0$, we can then predict future system state and outputs for all $t > t_0$. Importantly, many engineering and biological models can be described as a state-determined system model.  
    \end{block}
    % \begin{examples}
...
\begin{frame}{Cyber-physical systems and Safety-Critical Control}
    ...
    \end{itemize} 
    Yet, we can all agree that any engineered system should be designed as safe! (Eg. ACC)
    \bigbreak
    \textbf{New Goal:} Design a controller, or a combination of components, so that any system has some desired characteristics, but \textit{always maintains safety-critical properties}. A system that starts in a safe path or condition must never be able to access or reach an unsafe region.
    \bigbreak
    We call this \alert{invariance}.
\end{frame}
...
\begin{frame}{Sketch}
    \begin{figure}
    \includegraphics[width=0.6\linewidth]{Untitled 64.png}
    \end{figure}
...
\bigbreak
In an upcoming paper, we show that this easing of a restriction still guarantees that the members remain within the safe set. 
...
\begin{frame}{Numerical Example 1}
    \begin{figure}
    \includegraphics[width=0.5\linewidth]{OutputOfFig1CBFbeta20.jpg}
...
\begin{frame}{Numerical Example 2}
    \begin{figure}
    \includegraphics[width=0.5\linewidth]{OutputOfFig2CBFbeta20.jpg}
    \caption{The same initial conditions are used as in previos example. This time the nonsmooth CBF is implemented with $\beta_1=\beta_2=20$. All trajectories produced nonsmooth CBF controller stay within $C^*$. The smaller $\beta_1,\beta_2$ values forces the trajectories to move away from the boundary of $C^*$.}
    \end{figure}
\end{frame}
